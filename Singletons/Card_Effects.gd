...
func _ready():
	var _HV1 = SignalBus.connect("Card_Summoned", Callable(self, "c30093650")) # Fire (Tech)
	var _HV2 = SignalBus.connect("Card_Summoned", Callable(self, "c22716806")) # Mordred (Hero)
...
func c28269385(card): # Morgan le Fay (Hero)
	if On_Field(card) and GameData.Current_Phase == "Standby Phase" and GameData.Current_Step == "Effect":
		...
		if Check_Yield_Status():
			...
			if Flip_Result == 1:
				await SignalBus.Card_Effect_Selection_Yield_Release
...
func c17369913(card): # Last Stand (Magic/Equip)
	...
	# Update Field Card ACTUAL Stats
	if "EquipMagic" in card.get_parent().name and card.Effect_Active:
		card.Effect_Active = false
	...
	# Update Field Card Stat Visuals
	if Fighter != null:
		Fighter.Update_Data()
